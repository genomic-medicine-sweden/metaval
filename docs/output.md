...
Flag taxonomy tables generated by [taxpasta](https://github.com/taxprofiler/taxpasta) against matched negative controls. This step can help identify background noise or contaminants. It is only performed when `--flag_taxpasta` is supplied.

<details markdown="1">
<summary>Output files</summary>

- `taxpasta_flagged/`
  - `<sample_id>_<library_type>_<batch>_<classifier>.tsv`: Flagged taxonomy table for each sample and classifier.

</details>

For each classifier-specific taxpasta table, samples are compared against negative controls with the same `library_type` and `batch`. The output includes sample counts, NTC counts, and a comparison column with the following values:

- `in_sample`: reads are present only in the sample
- `in_NTC`: reads are present only in the matched negative control
- `more`: the sample count is higher than the matched negative control count
- `less`: the sample count is lower than the matched negative control count
- `equal`: the sample count is equal to the matched negative control count

If there is no negative control with the same `library_type` and `batch`, the pipeline still writes a separate taxonomy table for that sample, but without the NTC comparison columns.

The taxpasta table must contain the following columns: `taxonomy_id`, `name`, `rank`, and `lineage`.

If your taxpasta table was generated with [nf-core/taxprofiler](https://github.com/nf-core/taxprofiler), include the following parameters in your command:
...
If your taxpasta table was generated with `taxpasta` outside of nf-core/taxprofiler, include the following parameters when running `taxpasta`:
