...
| fastq_1             | Unmapped human reads from bowtie2/minimap2, filtered reads from bbduk/nanoq/FiltLong or raw sequencing reads. Gzipped compressed files accepted. Add a samplesheet entry for each FASTQ file belonging to this sample. They will be automatically merged by the pipeline. |
| fastq_2             | Unmapped human reads from bowtie2, filtered reads from bbduk/nanoq/FiltLong or raw reads. Gzipped compressed files accepted. Add a samplesheet entry for each FASTQ file belonging to this sample. They will be automatically merged by the pipeline.                     |
