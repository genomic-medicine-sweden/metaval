    ...
    function buildProteinAlignment(queryAa, subjectAa, qstart, qend, sstart, send, width = 60) {
      ...
      const lines = [
        `Query nt range: ${escapeHtml(`${queryNtStart}-${queryNtEnd}`)}`,
        "",
      ];
      let queryAaCursor = 1;
    ...
    function buildBlastAlignment(rowMap, width = 60) {
      ...
      if (!qseq || !sseq || !Number.isFinite(qstart) || !Number.isFinite(qend) || !Number.isFinite(sstart) || !Number.isFinite(send)) {
        return '<div class="blast-align-empty">No alignment sequence available for this hit.</div>';
      }

      if (isBlastxLikeAlignment(qseq, sseq)) {
        return buildProteinAlignment(
          translateBlastxQuery(qseq),
          sseq,
          qstart,
          qend,
          sstart,
    ...
    function renderDetailTable(headers, rows, options = {}) {
      if (!headers || !headers.length) {
        return options.emptyMessage
          ? `<div class="muted">${escapeHtml(options.emptyMessage)}</div>`
          : "";
      }
      ...
      const alignmentInsertIndex = options.blastAlignment
        ? Math.max(visibleIndices.findIndex((item) => item.key === "sscinames"), -1) + 1
        : -1;
      ...
      return `
        <div class="detail-table-wrap">
          <table${options.sortable ? ' data-sortable-blast' : ''}${tableClass}${options.blastAlignment ? ' data-blast-alignment="true"' : ''}>
            <thead>
              <tr>
                ${visibleIndices.map((item, visibleIndex) => `
            ...
            <tbody>
              ${rows.map((row, rowIndex) => {
                ...
                const rowMap = Object.fromEntries(headers.map((header, headerIndex) => [headerKeys[headerIndex], row[headerIndex] ?? ""]));
                return `
    ...
    function renderReadsSections(readsSections) {
      ...
      return readsSections.map((section) => `
    ...
    function renderOrganismPanel(detail) {
      ...
      const coveragePlots = detail.coverage_plot_sections && detail.coverage_plot_sections.length
        ? `
            <div class="coverage-plots-scroll">
              ${detail.coverage_plot_sections.map((section) => `
              <div class="plot">
      ...
      const coverageTables = detail.coverage_tables && detail.coverage_tables.length
        ? detail.coverage_tables.map((table) => `
            <div class="subsection">
    ...
    function populateFilterOptions(select) {
      ...
      for (const value of values) {
        ...
        option.value = value;
        option.textContent = key === "is_ntc"
          ? (value === "true" ? "NTC" : "Sample")
          : value;
    ...
    function applyFilters() {
      ...
      const activeRow = rows.find(
        (row) => row.classList.contains("is-active") && row.style.display !== "none"
      );
      if (!activeRow) {
    ...
    function updateSampleSortHeaders() {
      sampleSortHeaders.forEach((header) => {
        header.classList.toggle("is-asc", header.dataset.sortKey === sampleSortState.key && sampleSortState.direction === "asc");
        header.classList.toggle("is-desc", header.dataset.sortKey === sampleSortState.key && sampleSortState.direction === "desc");
      });
    }
    ...
    function applyTaxpastaFilter(view) {
      ...
      taxonomyRows.forEach((row) => {
        const matchesSuperkingdom =
          !filter || filter.value === "all" || row.dataset.superkingdom === filter.value;
        const matchesFlag =
          !flagFilter || flagFilter.value === "all" || row.dataset.flag === flagFilter.value;
    ...
    function sortDetailTable(table, index, direction) {
      ...
      if (table.dataset.blastAlignment === "true") {
        Array.from(tbody.querySelectorAll(".blast-summary-row"))
          .map((summaryRow) => ({
            summaryRow,
            detailRow: summaryRow.nextElementSibling?.classList.contains("blast-align-row") ? summaryRow.nextElementSibling : null,
          }))
          .sort((left, right) => {
    ...
    function restorePanelFromUrl() {
      const hashId = window.location.hash.startsWith("#")
      ...
      const hashSampleId = window.location.hash.startsWith("#sample-")
        ? window.location.hash.slice(1)
        : "";
